        ...
        const fetchData = async () => {
            try {
                const res = await fetch(
                    'http://localhost:4200/user/' + userAuth?.id,
                    {
                        credentials: 'include',
                    }
                );
                ...
                const strats = await fetch(
                    'http://localhost:4200/strategies/uploaded_by&' +
                        userAuth?.login,
                    {
                        credentials: 'include',
                    }
        ...
        return (
            <div className='card' style={{ width: '800px' }}>
                <div className={styles.summary}>
                    ...
                    <p>
                        Email:{' '}
                        <span className={styles.userText}>{data.email}</span>
                    </p>
                    <p>
                        Uploaded strategies:{' '}
                        <span className={styles.userText}>{stratCount}</span>
                    </p>
                    <p>
                        Joined:{' '}
